...
export async function GET(request) {
  ...
  let q = s.from('fiches_paie').select('*').eq('user_id', user.id)
...
export async function POST(request) {
  ...
  const { data, error } = await s.from('fiches_paie').insert(secured).select();
...
export async function DELETE(request) {
  ...
  const { error } = await s.from('fiches_paie').delete()
