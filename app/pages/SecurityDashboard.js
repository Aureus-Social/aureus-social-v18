...
export function SecurityDashboard({s,d,supabase,user,defaultTab}){
  ...
  return <div style={{padding:24}}>
    ...
    <div style={{display:'flex',gap:4,marginBottom:16,overflowX:'auto',paddingBottom:4,flexShrink:0}}>{[{v:'overview',l:'🛡 Vue d\'ensemble'},{v:'niveau1',l:'🔴 N1 ('+checks.filter(c=>c.level===1).length+')'},{v:'niveau2',l:'🟠 N2 ('+checks.filter(c=>c.level===2).length+')'},{v:'niveau3',l:'🟡 N3 ('+checks.filter(c=>c.level===3).length+')'},{v:'rgpd',l:'🔵 N4 ('+checks.filter(c=>c.level===4).length+')'},{v:'password',l:'🔑 Test mot de passe'},{v:'headers',l:'📋 Headers HTTP'},{v:'encryption',l:'🔒 Chiffrement'},{v:'rgpddocs',l:'📜 Documents RGPD'},{v:'ipwhitelist',l:'🌐 IP Whitelist'},{v:'backup',l:'💾 Backup données'},{v:'secpro',l:'🔐 Security Pro'}].map(t=><button key={t.v} onClick={()=>setTab(t.v)} style={{padding:'6px 10px',borderRadius:8,border:'none',cursor:'pointer',fontSize:11,flexShrink:0,whiteSpace:'nowrap',fontWeight:tab===t.v?600:400,fontFamily:'inherit',background:tab===t.v?'rgba(198,163,78,.15)':'rgba(255,255,255,.03)',color:tab===t.v?'#c6a34e':'#9e9b93'}}>{t.l}</button>)}</div>
