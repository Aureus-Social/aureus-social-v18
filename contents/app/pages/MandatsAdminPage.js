...
export default function MandatsAdminPage({ s, d, tab }) {
  ...
  // ── Charger données entreprise depuis state s ──
  const co = s?.co || {};
  const emps = s?.emps || [];
  const activeEmps = emps.filter(e => e.status !== 'sorti');

  // ── Sauvegarder mandat généré dans Supabase ──
  const saveMandatLog = async (type, data) => {
    if (!supabase || !s?.user?.id) return;
    await supabase.from('mandats_log').insert([{
      user_id: s.user.id,
      type,
      data: JSON.stringify(data),
      generated_at: new Date().toISOString(),
    }]).then(({ error }) => { if (error) console.warn('[Mandats] save:', error.message); });
  };
