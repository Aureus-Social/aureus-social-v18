...
-- Uniquement ce qui peut manquer, sans recréer ce qui existe déjà
-- Exécuter dans Supabase SQL Editor (projet jwjtlpewwdjxdboxtbdf)

-- ── Tables manquantes (IF NOT EXISTS OK sur tables) ────────────────
...
-- ── RLS — activer sur nouvelles tables seulement ───────────────────
...
-- ── Policies — DROP IF EXISTS puis recréer proprement ─────────────
...
CREATE INDEX IF NOT EXISTS idx_absences_dates ON absences(user_id, date_debut);
